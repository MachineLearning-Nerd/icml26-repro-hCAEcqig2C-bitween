...
def require_exact_domain(observed_rows: list[dict], label: str) -> None:
    """Require the complete, ordered 01..80 benchmark domain."""
    numbers = sorted(row["number"] for row in observed_rows)
    require(
        len(observed_rows) == 80,
        f"{label} requires 80 logs, observed {len(observed_rows)}",
    )
    require(numbers == list(range(1, 81)), f"{label} IDs must span 01 through 80")
...
def main() -> None:
    ...
    require_exact_domain(vanilla, "Claim 2")
    ...
    require_exact_domain(agentic, "Agentic evidence")
